...
class VeoService:
    ...
    async def extend_video(
        ...
        IMPORTANT: Only videos originally generated by Veo can be extended.
        Pass the veo_video_uri or veo_video_name from the original generation.

        Args:
            video_url: URL of the video to extend (GCS backup, used if no Veo ref)
        ...
        # IMPORTANT: Veo extension only works with videos generated by Veo
        # We must use the original Veo video reference, not re-upload
        if veo_video_uri:
            logger.info(f"Using original Veo video URI for extension: {veo_video_uri}")
            video_ref = types.Video(uri=veo_video_uri)
        elif veo_video_name:
            logger.info(f"Using original Veo video name for extension: {veo_video_name}")
            ...
            raise Exception(
                "Video extension requires the original Veo video reference. "
                "Only videos generated by Veo can be extended. "
                "Please provide veo_video_uri or veo_video_name from the original generation."
            )
        ...
        operation = await loop.run_in_executor(
            ...
            lambda: self.client.models.generate_videos(
                ...
                prompt=prompt,
                video=video_ref,  # Pass original Veo video reference
                config=config,
            ),
        )
